...
test('empty and prefilled commands, quoted locations, and aliases', () => {
  assert.deepEqual(validateSeed(parseCommand('/add'), types), {damage:0,damageType:'cr',armorDivisor:1,hitlocation:undefined});
  assert.deepEqual(validateSeed(parseCommand('/madd 12 cut location="Left Arm" divisor=2'),types), {damage:12,damageType:'cut',armorDivisor:2,hitlocation:'Left Arm'});
...
test('invalid damage, dice, types, options and quotes are rejected', () => {
  for (const command of ['/add 2d cut','/add -3 cr','/add 1.5 cr','/add 4 huh','/add 5 cr divisor=0','/add 5 cr divisor=Infinity','/add 5 cr loc=arm','/add 5 cr location="Left Arm','/add 9007199254740992 cr','/add 5 cr divisor=2 divisor=3']) {
    assert.throws(() => validateSeed(parseCommand(command),types),command);
  }
});

function token(id, actor, linked = false) {
  return {actor, document:{id,uuid:`Scene.s.Token.${id}`,actorLink:linked,name:id,parent:{id:'s'}}};
}
test('linked actors deduplicate; independent NPC copies remain separate', () => {
  const actor={id:'a',uuid:'Actor.a',isOwner:true};
  const result=collectRecipients([token('a',actor,true),token('b',actor,true),token('c',actor),token('d',actor)],{isGM:true},true);
...
test('recipient body plan is respected and Random does not roll on opening', () => {
  const actor={name:'NPC',defaultHitLocation:'Random',hitLocationsWithDR:[{where:'Torso'},{where:'Left Arm'}]};
