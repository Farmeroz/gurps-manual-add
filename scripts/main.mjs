...
let active = null, DialogClass = null, initialising = false;
const HELP = '/add [damage] [type] [location="Left Arm"] [divisor=2]. Select recipient tokens first. Example: /add 12 cut location="Left Arm" divisor=2. /madd is an alias. No dice are rolled.';

function setting(key) { return game.settings.get(ID, key); }

async function initialiseDialog() {
  if (DialogClass) return;
  if (game.system.id !== 'gurps' || !/^0\.18\./.test(game.system.version)) {
    throw new Error('This release supports GGA 0.18.x. A changed GGA API needs a compatibility review.');
  }
  const NativeADD = globalThis.GURPS?.ApplyDamageDialog ??
    (await import(foundry.utils.getRoute('systems/gurps/module/damage/applydamage.js'))).default;
  for (const method of ['getData', 'activateListeners', 'submitInjuryApply', 'resolveInjury', '_renderTemplate']) {
    if (typeof NativeADD?.prototype?.[method] !== 'function') throw new Error(`GGA ADD is missing ${method}.`);
  }
...
export async function open(options = {}) {
  ...
  try {
    ...
    if (gmOnly && !game.user.isGM) throw new Error('GGA is configured to allow only GMs to open the ADD.');
    const seed = validateSeed(options, GURPS.DamageTables.woundModifiers);
    const supplied = options.tokens ?? canvas.tokens.controlled;
    const tokens = Array.from(supplied, token => typeof token === 'string' ? canvas.tokens.get(token) : token).filter(Boolean);
    ...
    if (skipped.length) ui.notifications.warn(`Manual damage skipped ${skipped.length} token(s) without an owned actor or permission.`);
    if (duplicates.length) ui.notifications.info(`Manual damage: ${duplicates.length} duplicate linked token(s) omitted; each shared actor is included once.`);
    if (!recipients.length) throw new Error('No permitted recipient actors were selected.');
    await initialiseDialog();
    const session = new RecipientSession(DialogClass, recipients, seed, () => { active = null; });
    active = session;
    initialising = false;
    session.show();
...
export async function command(line) {
  try {
    ...
    if (options.help) { ui.notifications.info(HELP, { permanent: true }); return true; }
...
Hooks.once('init', () => {
  game.settings.register(ID, 'enabled', {
    name: 'Enable manual damage', hint: 'Allow opening GGA’s full ADD without a damage roll. GGA’s ADD permissions still apply.',
    scope: 'world', config: true, type: Boolean, default: true,
  });
  game.settings.register(ID, 'hudButton', {
    name: 'Show Manual Damage on token HUD', hint: 'Add a calculator button and a Manual Damage entry to supported token context menus.',
...
Hooks.once('ready', () => {
  ...
  if (typeof registry?.registerProcessor === 'function') {
    ...
    const aliases = ['add', 'madd'].filter(alias => {
      const conflict = existing.some(p => p.matches(`/${alias}`) || p.matches(`/${alias} 12 cut`));
      if (conflict) ui.notifications.warn(`Manual damage: /${alias} is already registered; use the other alias or module API.`);
      return !conflict;
    });
    const processor = {
      registry: null,
      ...
      usagematches: () => false,
      help: () => aliases.length ? `/${aliases[0]} [damage] [type] – open Manual Damage for selected tokens` : null,
      isGMOnly: () => false,
      process: line => command(line),
    };
    if (aliases.length) registry.registerProcessor(processor);
  } else {
    ui.notifications.warn('Manual damage: GGA chat integration is unavailable. Use the HUD or JavaScript API.');
  }
